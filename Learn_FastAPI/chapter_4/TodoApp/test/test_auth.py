"""
Tests for authentication-related functions and endpoints in the FastAPI application.
...
def test_create_access_token() -> None:
    """Test creation and decoding of JWT access token."""
    username = "testuser"
    user_id = 1
    role = "user"
...
async def test_get_current_user_valid_token() -> None:
    """Test retrieving user from a valid JWT token."""
...
@pytest.mark.asyncio
async def test_get_current_user_missing_payload() -> None:
    """Test error handling when JWT token is missing required payload fields."""
    encode = {"role": "user"}
    token = jwt.encode(encode, SECRET_KEY, algorithm=ALGORITHM)
